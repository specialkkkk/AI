#36_1 복붙
#reshape를 없애고 input_shape(28,28)로 고쳤다.
...
model.add(Dropout(0.3))                                        #  ----------->  Flatten을 원래 여기서 써서 인풋만 펴서(Flatten) 2차원으로 만들었었다.
model.add(Dense(64, activation='relu'))
model.add(Dropout(0.3))
model.add(Dense(32, activation='relu'))
model.add(Flatten())                                    # ------------->    Dense모델은 사실 2차원 이상 가능하다 /  Flatten을 마지막쯤에 해도 상관없고 결과값이 좋게 나왔을때의 위치가 좋은것
...
mcp = ModelCheckpoint(monitor='val_loss', mode='auto', verbose=1,
                    ...
                    filepath= filepath + 'k37_01_' + date + filename)
...
model.save(path + "keras37_1_mnist_save_mode.h5")
